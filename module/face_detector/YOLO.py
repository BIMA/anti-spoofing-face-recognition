...
class YOLOv5:
    def __init__(
        self, weights: str = None, input_res: tuple = (640, 640), batch_size: int = 1
    ):
        super().__init__()
    ...
    def _init_session_(self, path_onnx_model: str):
        ...
        if os.path.isfile(path_onnx_model):
            ort_session = ort.InferenceSession(
                path_onnx_model, providers=["CPUExecutionProvider"]
            )
    ...
    def preprocessing(self, imgs: list):
        imgs_input = []
        for img in imgs:
            img_input, ratio, (dw, dh) = letterbox(
                img,
                self.input_res,
                auto=False,
                scaleFill=False,
    ...
    def postprocessing(
        self, prediction_bboxes, imgs, conf_thresh=0.25, iou_thresh=0.1, max_detection=1
    ):
        assert len(prediction_bboxes) == len(
            imgs
        ), f"Size prediction {len(prediction_bboxes)} not equal size images {len(imgs)}"
        ...
        for i, det in enumerate(pred):  # detections per image
            if len(det):
                det[:, :4] = scale_coords(
                    self.input_res, det[:, :4], imgs[i].shape
                ).round()
        return pred
